...
begin
  ...
  if normalized_role = 'customer' then
    -- Backward compatibility: support single property_id if present
    begin
      meta_property_id := nullif(new.raw_user_meta_data ->> 'property_id', '')::uuid;
    exception
      when others then
        meta_property_id := null;
    end;

    if meta_property_id is not null then
    ...
    -- New behavior: support multiple property_ids
    if jsonb_typeof(new.raw_user_meta_data -> 'property_ids') = 'array' then
      for property_id_text in
        select jsonb_array_elements_text(new.raw_user_meta_data -> 'property_ids')
      loop
        begin
          insert into public.property_customers (
            property_id,
